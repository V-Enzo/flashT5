...
class FlashT5Stack(nn.Module, ModuleUtilsMixin):
    def __init__(self, config, embed_tokens, pkt_token_proj:nn.Embedding=None, seg_proj:nn.Embedding=None):
        super().__init__()
        ...
        self.is_decoder = config.is_decoder
        self.pkt_token_proj = pkt_token_proj
    ...
    def forward(
        ...
        # Add packet and head token embeddings
        # pkt embed_ids should has dummy token -100 for non-packet tokens or use 0
        # head embed_ids should has dummy token -100 for non-head tokens or use 0
        if self.pkt_token_proj is not None and pkt_seg_ind is not None:
            pkt_embeds = self.pkt_token_proj(pkt_seg_ind)
            # TODO: check what was added to the input_embeds.
            inputs_embeds = inputs_embeds + pkt_embeds
        
        if self.seg_proj is not None and head_payload_seg_ind is not None:
            seg_embeds = self.seg_proj(head_payload_seg_ind)
            # TODO: check what was added to the input_embeds.
            inputs_embeds = inputs_embeds + seg_embeds
...
class FlashT5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, FlashT5ForPretrainTasks):
            module.shared.weight.data.normal_(mean=0.0, std=factor * 1.0)
            if hasattr(module, "lm_head") and not self.config.tie_word_embeddings:
                module.lm_head.weight.data.normal_(mean=0.0, std=factor * self.config.d_model ** -0.5)
            if hasattr(module, "pop_mlp"):
                module.pop_mlp.weight.data.normal_(mean=0.0, std=factor * self.config.d_model ** -0.5)
            if hasattr(module, "nml_mlp_link_layer"):
                module.nml_mlp_link_layer.weight.data.normal_(mean=0.0, std=factor * self.config.d_model ** -0.5)
            if hasattr(module, "nml_mlp_network_layer"):
                module.nml_mlp_network_layer.weight.data.normal_(mean=0.0, std=factor * self.config.d_model ** -0.5)
            if hasattr(module, "nml_mlp_transport_layer"):
                module.nml_mlp_transport_layer.weight.data.normal_(mean=0.0, std=factor * self.config.d_model ** -0.5)
            if hasattr(module, "nml_mlp_app_layer"):
                module.nml_mlp_app_layer.weight.data.normal_(mean=0.0, std=factor * self.config.d_model ** -0.5)   
...
class FlashT5Model(FlashT5PreTrainedModel):
    ...
    def __init__(self, config: FlashT5Config):
        ...
        # 10 pkts need to set to 11, as the -100 will generate the 11th packet
        self.pkt_embed = nn.Embedding(11, config.d_model)
        self.head_embed = nn.Embedding(2, config.d_model)
        
        encoder_config = copy.deepcopy(config)
        encoder_config.is_decoder = False
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        # Only encoder embed the pkt and head tokens
        self.encoder = FlashT5Stack(encoder_config, self.shared, self.pkt_embed, self.head_embed)
    ...
    def forward(
        ...
        only_encoder: Optional[bool] = False,
        ...
        # If only_encoder is True, return the encoder hidden states only
        if only_encoder is None and decoder_input_ids is None:
            return ModelEncOutput(encoder_hidden_states=hidden_states)
...
@dataclass
class ModelEncOutput(ModelOutput):
    encoder_hidden_states: torch.FloatTensor = None
    
@dataclass
class PretrainTasksOutput(ModelOutput):
    msp_loss: Optional[torch.FloatTensor] = None
    msp_logit: Optional[torch.FloatTensor] = None
    msp_label: Optional[torch.FloatTensor] = None
    pop_loss: Optional[torch.FloatTensor] = None
    pop_logit: Optional[torch.FloatTensor] = None
    pop_label: Optional[torch.FloatTensor] = None
    nml_loss: Optional[torch.FloatTensor] = None
    nml_logit: Optional[torch.FloatTensor] = None
    nml_label: Optional[torch.FloatTensor] = None
    
# For pretraining tasks include the following lm head, packet order prediction, network model layer prediction 
class FlashT5ForPretrainTasks(FlashT5PreTrainedModel):
    ...
    def __init__(self, config: FlashT5Config):
        ...
        # 10 pkts need to set to 11, as the -100 will generate the 11th packet
        self.pkt_token_proj = nn.Embedding(11, config.d_model)
        self.seg_proj = nn.Embedding(2, config.d_model)

        encoder_config = copy.deepcopy(config)
        encoder_config.is_decoder = False
        # Only encoder embed the pkt and head tokens
        self.encoder = FlashT5Stack(encoder_config, self.shared, self.pkt_token_proj, self.seg_proj)
    ...
    def cal_msp_loss(self, 
                     decoder_outputs: Tuple[torch.FloatTensor, ...], 
                     labels: Optional[torch.LongTensor] = None,
    ...
    def cal_pop_loss(self,
                     enc_hidden_states: Optional[torch.FloatTensor],
                     pkt_order_labels: Optional[torch.LongTensor] = None,
                     pkt_order_mask: Optional[torch.BoolTensor] = None,
        )-> torch.FloatTensor:
        valid_pkt_order_labels = pkt_order_labels[pkt_order_labels != -100]
        num_labels = valid_pkt_order_labels.numel()
        pkt_order_mask = pkt_order_mask.unsqueeze(-1).expand_as(enc_hidden_states)
        pkt_order_hidden_states = enc_hidden_states[pkt_order_mask].view(-1, self.config.d_model)
        pkt_order_logits = self.pop_mlp(pkt_order_hidden_states)
        if num_labels > 0:
            packet_order_loss = F.cross_entropy(pkt_order_logits, valid_pkt_order_labels)
        else:
            packet_order_loss = torch.tensor(0.0, device=pkt_order_logits.device, requires_grad=False) * pkt_order_logits.sum()
        
        return packet_order_loss, pkt_order_logits, valid_pkt_order_labels
        ...
    
    def cal_nml_loss(self,
                     enc_hidden_states: Optional[torch.FloatTensor],
                    nml_labels: Optional[torch.LongTensor] = None,
                    nml_mask: Optional[torch.BoolTensor] = None,
        )-> torch.FloatTensor:
        
        valid_nml_labels = nml_labels[nml_labels != -100]
        idx_range = torch.arange(0, valid_nml_labels.numel(), step=4)
        nml_link_label = valid_nml_labels[idx_range]
        nml_network_label = valid_nml_labels[idx_range + 1]
        nml_transport_label = valid_nml_labels[idx_range + 2]
        nml_app_label = valid_nml_labels[idx_range + 3]
        
        nml_mask = nml_mask.unsqueeze(-1).expand_as(enc_hidden_states)
        network_model_hidden_states = enc_hidden_states[nml_mask].view(-1, self.config.d_model)
        nml_linklayer_logits = self.nml_mlp_link_layer(network_model_hidden_states)
        nml_linklayer_loss = F.cross_entropy(nml_linklayer_logits, nml_link_label)
        
        nml_networklayer_logits = self.nml_mlp_network_layer(network_model_hidden_states)
        nml_networklayer_loss = F.cross_entropy(nml_networklayer_logits, nml_network_label)
        
        nml_transportlayer_logits = self.nml_mlp_transport_layer(network_model_hidden_states)
        nml_transportlayer_loss = F.cross_entropy(nml_transportlayer_logits, nml_transport_label)
        
        nml_applayer_logits = self.nml_mlp_app_layer(network_model_hidden_states)
        nml_applayer_loss = F.cross_entropy(nml_applayer_logits, nml_app_label)
        
        # TODO: Should divide the sum by 4 to get the average loss.
        nml_loss = (nml_linklayer_loss + nml_networklayer_loss + nml_transportlayer_loss + nml_applayer_loss)
        
        assert not torch.isnan(nml_loss).any(), "NML loss is NaN"
        
        nml_logits = (nml_linklayer_logits, nml_networklayer_logits, nml_transportlayer_logits, nml_applayer_logits)
        
        nml_labels = (nml_link_label, nml_network_label, nml_transport_label, nml_app_label)
        
        return nml_loss, nml_logits, nml_labels
        
    
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        pkt_seg_ind: Optional[torch.LongTensor] = None,
        head_payload_seg_ind: Optional[torch.LongTensor] = None,
        attention_mask: Optional[torch.FloatTensor] = None,
        decoder_input_ids: Optional[torch.LongTensor] = None,
        decoder_attention_mask: Optional[torch.BoolTensor] = None,
        labels: Optional[torch.LongTensor] = None,
        encoder_hidden_states: Optional[torch.FloatTensor] = None,
        pop_order: Optional[torch.LongTensor] = None,
        pop_mask: Optional[torch.BoolTensor] = None,
        nml_labels: Optional[torch.LongTensor] = None,
        nml_mask: Optional[torch.BoolTensor] = None,
    ) -> Seq2SeqLMOutput:
        """
        ...
        if encoder_hidden_states is None:
            encoder_hidden_states = self.encoder(
                input_ids=input_ids,
                pkt_seg_ind=pkt_seg_ind,
                head_payload_seg_ind=head_payload_seg_ind,
                attention_mask=attention_mask,
            )[0]

        hidden_states = encoder_hidden_states

        pop_loss, pop_logits, pop_labels = self.cal_pop_loss(encoder_hidden_states, pop_order, pop_mask)
        
        nml_loss, nml_logits, nml_labels = self.cal_nml_loss(encoder_hidden_states, nml_labels, nml_mask)
        ...
        msp_loss, msp_logits = self.cal_msp_loss(decoder_outputs, labels)


        return PretrainTasksOutput(
                msp_loss=msp_loss,
                msp_logit=msp_logits,
                msp_label = labels,
                pop_loss=pop_loss,
                pop_logit=pop_logits,
                pop_label = pop_labels,
                nml_loss=nml_loss,
                nml_logit=nml_logits,
                nml_label = nml_labels
        )